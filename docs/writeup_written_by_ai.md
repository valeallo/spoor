...
- **Speed vs. Accuracy**: I opted for `yolov8s` to ensure the prototype runs quickly while remaining accurate.
- **Pre-trained Limitations**: The COCO "bird" class is generic. It will detect many birds but might struggle with specific species, small birds at a distance, or challenging lighting/weather, which are common in real wind farm deployments. To counteract this, I built a Human-In-The-Loop Annotation Studio directly into the app so users can dynamically fine-tune the model on new environments.
- **Tracking Algorithm**: BoT-SORT is used. By default it forgets lost tracks too quickly, so I implemented a `custom_tracker.yaml` to extend the track buffer, but it can still struggle with severe long-term occlusions.
